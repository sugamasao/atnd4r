...
* 2009/7/24 時点では、APIが正式公開前のため{WebAPI の解説ページ}[http://api.atnd.org] にアクセス制限が掛かっています（API自体は動いてるけど）。
...
基本的には XML データを Ruby 用のオブジェクトに変換したものですので、プロパティエなどはほぼそのままです（ただし、ハイフンのものはアンダースコアにしています）。
また、各プロパティは XML の属性にしたがったオブジェクトに変換されています。例えば、Integer とあれば Fixnum 型のように。

==== 以下、オブジェクト構成

カッコの既述があるもの意外は String や Fixnum のようなRuby標準の型になっています。
